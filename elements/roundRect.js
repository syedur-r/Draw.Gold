// constructor for the round rectangle tool
function RoundRect() {
   Rectangle.call(this); // Inherits all the methods and properties from the Rectangle constructor

   this.icon = "graphics/roundRect.png";
   this.name = "Round Rectangle Tool";
   ...
   this.draw = function () { // This starts the draw method on the constructor function allowing more interaction to take place as well as overriding the properties of the variables
      ...
      if (bounds.mousePressOnCanvas(canvasContent) && mouseIsPressed) {
         ...
         if (this.selectorMode == false) {
            ...
            //if we already have values for mouseOrigin.x and mouseOrigin.y we can draw a round rectangle from 
            //there to the current mouse location
            } else {
               updatePixels(); // Updates the display window with the data in the pixels array
               push(); // push and pop contains the style settings of the round rectangle
               ...
               rect(this.mouseOrigin.x, this.mouseOrigin.y, mouseX - this.mouseOrigin.x, mouseY - this.mouseOrigin.y, 15); // draws a  round rectangle from the previous mouse position to the current mouse position, the previous position starts from the top corner of the round rectangle
               pop();
            }
         }
         ...
         // If nothing is drawn on the canvas, the origin of mouse X and Y  is set to -1 (off the canvas) and the default state of drawing the round rectangle is set to false as the mouse state is released on the canvas
      }
   };
}